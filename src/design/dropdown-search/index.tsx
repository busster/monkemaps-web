...
},
{
  actions: {
    ...
    handleSelectResult: (context, event) => props.onSelect((event as any).value)
  },
  guards: {
...
export const MDDropdownSearch = (props: MDDropdownSearchProps) => {
  ...
  return (
    <div ref={rootRef} className='md-dropdown-search__container'>
      {label && (
        <label
          className='md-dropdown-search__label'
          htmlFor={inputId}
        >{ label }</label>
      )}
      {
      ...
      <div
        ref={popoverRef}
        className={`md-dropdown-search__results ${['results', 'searching'].some(state.matches) ? 'md-dropdown-search__results--show' : ''}`}
      >
        {
          state.context.results.length > 0 ?
            state.context.results.map(result => (
              <div
                ...
                key={getId(result)}
                className={`md-dropdown-search__results-item ${isSelected(result) ? 'md-dropdown-search__results-item--selected' : ''}`}
              >
                { mapTextValue(result) }
              </div>
            )) :
