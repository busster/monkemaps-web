    ...
    return (
      <div className="Map-LocationList">
        <div className="Map-LocationList__tabs">
          <div
            className={`Map-LocationList__tab ${
              view === 'Physical' ? 'Map-LocationList__tab--active' : ''
            }`}
          ...
          <div
            className={`Map-LocationList__tab ${
              view === 'Virtual' ? 'Map-LocationList__tab--active' : ''
            }`}
        ...
        <div className="Map-LocationList__list">
          {view === 'Physical' ? (
            locations.length === 0 ? (
              ...
              locations.map((pin) => (
                <div
                  id={`Map-LocationList__item-${pin.id}`}
                  key={pin.id}
                  className={`Map-LocationList__item ${
                    pin.id === activeLocation?.id
                      ? 'Map-LocationList__item--active'
                      : ''
                  }`}
                >
                  <Link
                    className="Map-LocationList__item-link"
                    to={`/map/${pin.id}`}
                  >
                    <div className="Map-LocationList__item-container">
                      <div className="Map-LocationList__item-header">
                        {pin.name}
                      </div>
                      <div className="Map-LocationList__item-address">
                        {pin.text}
                      </div>
                      <div className="Map-LocationList__item-date">
                        {pin.startDate.toFormat('ccc, LLL dd, yyyy, hh:mm a')}
                      </div>
                    </div>
                  </Link>
                </div>
