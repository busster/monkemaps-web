...
const useMap = () => {
  ...
  const [clusterizer, setClusterizer] = useState(new Supercluster({
    radius: 20,
  }));
        ...
        if (props.markerType === 'Pin') {
          ...
          const marker = new mapboxgl.Marker(
            Marker({
              ...
              handleOnmouseenter: () => {
                document.getElementById(`Map-LocationList__item-${props.id}`)?.scrollIntoView({ behavior: 'smooth' });
              },
              handleOnmouseleave: () => {},
            })
          ).setLngLat(coords);
  ...
  handleSetVisibleMarkers = () => {
    ...
    setVisibleMarkers(state.context.pins.filter((pin) => markerIdsInBounds.includes(pin.id)))
  };
  const moveFunc = () => {
    handleSetLatLngZoom();
  }
  ...
  useEffect(() => {
    ...
    if (
      ([
        'data.loaded',
        'display.map'
      ]).every((substate) => state.matches(substate))
    ) {
      const allPinsArr = [
      ...
      clusterizer.load(allPinsArr.map(p => ({
        type: 'Feature',
        properties: {
          id: p.id,
...
export const Map = () => {
  const { container, visibleMarkers, state, reload, rerender, hoveredMarker, map } = useMap();
